...
| Maybe Happy Ending | 2024 | The one-act two-hander — countdown pacing vs. Spelling Bee's elimination; a ring ending that replays scene one |
| Hercules (West End) | 2025 | Screen-to-stage — a front-loaded film score leaves Act Two to be written from scratch; a narrator chorus promoted into structure |
...
| Book export — EPUB + print PDF, front/back matter, themes, trim sizes (see below; Manuscript-format PDF for agent submission stays as-is) | ✅ Phases 0–5 done (data model, front/back-matter editors, Book view render + PDF, self-hosted serif fonts, four theme presets + chapter-label styles + drop/raised/small-caps openers + scene-break ornaments, real book trim sizes 5×8–6×9, recto/verso mirrored gutters + running heads + roman/arabic book folios + chapter-start-recto blanks, full EPUB export — dependency-free stored-ZIP writer, cover→front-matter→chapters→back-matter with metadata/cover/nav, cover-upload control + Book-view "Download EPUB" button, and 2026-07-20: custom-theme knobs — opener size, expanded scene-break ornaments, paragraph indent-vs-block, line spacing — with a live in-drawer preview, honored in both the Book render and EPUB) — remaining: saved named themes (deferred, needs go-ahead) + a formal epubcheck pass (no local JRE); see `BOOK-FORMATTING-PLAN.md` |
