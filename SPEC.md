...
| Book export — EPUB + print PDF, front/back matter, themes, trim sizes (see below; Manuscript-format PDF for agent submission stays as-is) | 🔶 Phases 0–3b done (data model, front/back-matter editors, Book view render + PDF, self-hosted serif fonts, four theme presets + chapter-label styles + drop/raised/small-caps openers + scene-break ornaments, real book trim sizes 5×8–6×9, and 2026-07-20: recto/verso mirrored gutters + running heads + roman/arabic book folios + chapter-start-recto blanks) — EPUB (4a/4b) + custom themes (5) remain; see `BOOK-FORMATTING-PLAN.md` |
